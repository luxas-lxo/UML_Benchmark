...
class SemanticsEvaluator:
    ...
    def evaluate_classes(criteria: ScoringCriteria, model: EvalModel) -> ScoringCriteria:
        ...
        total_score = len(model.class_match_map)
        score: float = 0.0
        if model.instructor_model.class_list:
            for cls_i, cls_s in model.class_match_map.items():
                if SyntacticCheck.syntactic_match(cls_i, cls_s) or SemanticCheck.semantic_match(cls_i, cls_s):
                    score += 1
            class_completeness_score = score / total_score if total_score > 0 else 1.0
